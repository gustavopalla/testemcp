...
class TestePallaSupabaseUser extends BaseAuthUser {
  TestePallaSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> testePallaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TestePallaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
